...
export const useTsunamiCalculator = () => {
  ...
  const [travelTimes, setTravelTimes] = useState<TravelTimeResponse | null>(
    null,
  );
      ...
      if (data.status === 'completed') {
        ...
        if (data.started_at) {
          ...
          const progressPercentage = Math.min(
            Math.round((elapsedMinutes / estimatedTotalTime) * 90),
            90,
          );

          setProgress(progressPercentage);
          setEstimatedTimeRemaining(
            Math.max(estimatedTotalTime - elapsedMinutes, 0),
          );
        }
  ...
  const calculateTsunami = useCallback(
    async (values: GenerateFormType) => {
      ...
      try {
        ...
        const travelTimesResponse = await fetch(
          `${API_BASE_URL}/tsunami-travel-times`,
          {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
        ...
        if (!travelTimesResponse.ok) {
          ...
          throw new Error(
            errorData.message || 'Error en el cálculo de tiempos de viaje',
          );
        }
        ...
        if (!runTsdhnResponse.ok) {
          ...
          throw new Error(
            errorData.message || 'Error al iniciar la simulación',
          );
        }
